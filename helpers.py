...
def get_datasets_and_converter(args, force_no_random_crop=False):
    ...
    if "DAS" in (args.train_dataset, args.test_dataset):
        converter = conversion.Converter("image")


        train_dataset = dataset_.DASBigFileDataset(

            file_path='/cutdata_32*32_0501/TrainData.npy'   # 32*32

        )
        test_dataset = dataset_.DASBigFileDataset(

            file_path='/cutdata_32*32_0501/TestData.npy'   # 32*32
