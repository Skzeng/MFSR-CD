...
def compute_modulations(
    ...
    for i, data0 in enumerate(dataloader):
        print(f"Batch {i + 1}/{len(dataloader)}")

        if timing != "off":
            start = time.time()

        # data, filename, ori_data = data0
        filname, data = data0
        data = data.float()
        # print(filename)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        # default=1,    # train dataset
        default=0,   # test dataset
    )
    ...
    end_time = time.time()
    print("压缩时间:%.4f秒" % (end_time-start_time))
