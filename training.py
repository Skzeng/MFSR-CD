...
import metalearning as metalearning
import torch
import wandb
...
class Trainer:
    ...
    def train_epoch(self):
        ...
        for data0 in self.train_dataloader:
            # print(data[0])
            ...
            # print(features[0])
            # print('000000000', max(data[0]), min(data[0]))
            # print(max(features[0]), min(features[0]))
            ...
            print(
                f'Step {self.step}, Loss {log_dict["loss"]:.6f}, PSNR {log_dict["psnr"]:.3f}'
            )
    ...
    def validation(self):
        ...
        for inner_steps in self.args.validation_inner_steps:
            ...
            for i, data0 in enumerate(self.test_dataloader):
                # data, filname, ori_data = data0
                filname, data = data0
                data = data.float()
                data = data.to(self.args.device)
            ...
            print(
                f"Inner steps {inner_steps}, Loss {mean_loss:.6f}, PSNR {mean_psnr:.3f}"
            )
